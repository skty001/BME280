...
#define ADDR_RESET      0xE0  // リセットレジスタ
...
void writeRegister(unsigned char addr, unsigned char data) {
  smW.beginTransmission(BME280_ADDRESS); // SDO:LOW → 0x76
  smW.write(addr);
  smW.write(data);
  smW.endTransmission();
}
...
void readData() {
  ...
  smW.beginTransmission(BME280_ADDRESS); // 送信開始
  smW.write(0xF7);                       //
  smW.endTransmission();                 // 送信完了
  smW.requestFrom(BME280_ADDRESS, 8);    // 受信開始
  while (smW.available()) {              // 8byte受信まで
    data[i] = smW.read();
    i++;
  }
...
void readCalibration(){
  ...
  smW.write(0x88);
  smW.endTransmission();                 // 送信完了
  smW.requestFrom(BME280_ADDRESS, 25);   // 0x88→0xA1
  while(smW.available()){
    data[i] = smW.read();
    i++;
  }
  ...
  smW.write(0xE1);
  smW.endTransmission();                 // 送信完了
  smW.requestFrom(BME280_ADDRESS, 7);    // 0xE1→0xE8
  while(smW.available()){
    data[i] = smW.read();
    i++;
  }
